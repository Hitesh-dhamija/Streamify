...
import { setFavoriteList } from "../../stored";
...
function DetailsMovie() {
  ...
  const handleAddToFavorites = async () => {
    ...
    if (favoriteList?.length) {
      const movieExist = favoriteList.some?.(
        (item) => item.movie?.id === data?.id,
      );

      if (movieExist) {
        return toast.error("Movie already exist in favourite List!");
      }
    }

    setLoadingAddMovie(true);
    const newFavorite = await addMovieFromPlaylist(user.uid, data, media_type);
    dispatch(setFavoriteList([...favoriteList, newFavorite]));
